...
function typeWriterEffect() {
    ...
    // [W-img] OLF [S-img] PACE
    element.innerHTML = '<img src="https://i.ibb.co/F4ckQ0yV/W.png" alt="W" class="hero-w-img">';

    const part1 = "OLF ";   // texto antes de la S
    const part2 = "PACE";   // texto después de la S
    ...
    function typePart1() {
        if (i < part1.length) {
            span1.textContent += part1.charAt(i++);
            setTimeout(typePart1, 100);
        } else {
            // Insertar imagen S
            const sImg = document.createElement('img');
            sImg.src = 'https://i.ibb.co/j9QnVMpD/S.png';
            sImg.alt = 'S';
            sImg.className = 'hero-s-img';
            element.appendChild(sImg);
            ...
            function typePart2() {
                if (j < part2.length) {
                    span2.textContent += part2.charAt(j++);
...
function navigate(moduleName, action) {
    ...
    // Configurar icono y texto según el tipo de navegación
    const loadingLabel = document.getElementById('loading-label');
    const loadingIcon  = document.getElementById('loading-icon');
    if (loadingIcon)  loadingIcon.className = 'fa-solid fa-signal loading-icon-spin';
    if (loadingLabel) loadingLabel.textContent = 'ESTABLECIENDO CONEXIÓN...';
...
// --- Theme Skin Logic ---
const themes = ['default', 'cyberpunk', 'blood', 'matrix', 'gold'];
...
window.setTheme = function(nextTheme) {
    const loadingScreen  = document.getElementById('loading-screen');
    const loadingAvatar  = document.querySelector('.loading-avatar');
    const loadingLabel   = document.getElementById('loading-label');
    const loadingIcon    = document.getElementById('loading-icon');

    if (loadingScreen) {
        // Ocultar el avatar (imagen del módulo) — no aplica al cambiar skin
        if (loadingAvatar) loadingAvatar.style.display = 'none';
        if (loadingLabel) loadingLabel.textContent = 'CONFIGURANDO ENTORNO...';
        if (loadingIcon)  loadingIcon.className = 'fa-solid fa-palette loading-icon-spin';
        loadingScreen.classList.add('show');
    }
    ...
    setTimeout(() => {
        ...
        localStorage.setItem('wolfspace_theme', nextTheme);

        if (nextTheme === 'matrix') setTimeout(startMatrixRain, 100);

        markActiveTheme(nextTheme);

        setTimeout(() => {
            if (loadingScreen) {
                loadingScreen.classList.remove('show');
                // Restaurar avatar para próximas navegaciones
                setTimeout(() => {
                    if (loadingAvatar) loadingAvatar.style.display = '';
                    if (loadingLabel) loadingLabel.textContent = 'ESTABLECIENDO CONEXIÓN...';
                    if (loadingIcon)  loadingIcon.className = 'fa-solid fa-palette loading-icon-spin';
                }, 300);
            }
...
function markActiveTheme(activeKey) {
    const opts = document.querySelectorAll('.theme-option[data-theme-key]');
    opts.forEach(opt => {
        if (opt.dataset.themeKey === activeKey) {
...
document.addEventListener('DOMContentLoaded', () => {
    const savedTheme = localStorage.getItem('wolfspace_theme') || 'default';
